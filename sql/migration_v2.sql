...
drop policy if exists "reviews_select_buyer" on reviews;
drop policy if exists "reviews_select_seller" on reviews;
drop policy if exists "reviews_insert_buyer" on reviews;

create policy "reviews_select_buyer"
on reviews for select using (buyer_id = auth.uid());

create policy "reviews_select_seller"
on reviews for select using (seller_id = auth.uid());
